...
def calculate_trend_pullback_breakout(price, ma200, higher_high, higher_low, support, ema20, ema50, volume, breakout_volume):
    score = 0
    details: List[str] = []
    ...
    if price > ma200:
        score += 1
        details.append("Above 200 DMA✔")

    # Check Higher Highs and Higher Lows pattern
    if higher_high and higher_low:
        score += 1
        details.append("Higher Highs & Lows✔")

    # Check pullback to support or 20/50 EMA
    if (price > support) or (price > ema20) or (price > ema50):
        score += 1
        details.append("Pullback✔")

    # Check breakout with volume
    if volume > breakout_volume:
        score += 1
        details.append("Breakout Volume✔")
